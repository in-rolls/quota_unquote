# 01e_up_pai_prepare.R
# Prepare the Uttar Pradesh GP-theme table from the pinned PAI release.
...
message("Reading the PAI release for Uttar Pradesh")
scores <- read_pai_state_long(STATE_UP)

assert_unique(scores, c("year", "gp_code", "theme_slug"), "Uttar Pradesh PAI scores")
assert_unique(
    scores,
    c("year", "district", "block", "gp_name", "theme_slug"),
    "Uttar Pradesh PAI name keys"
)
if (!PAI_T8_SLUG %in% scores$theme_slug) {
    stop("The stable PAI Good Governance slug is absent for Uttar Pradesh", call. = FALSE)
}
if (any(is.na(scores$gp_code))) {
    stop("Every scored Uttar Pradesh PAI row must carry an LGD GP code", call. = FALSE)
}
...
expected_rows <- c("2022-2023" = 23207L, "2023-2024" = 57678L)
observed_rows <- setNames(profile$rows, profile$year)
if (!identical(observed_rows[names(expected_rows)], expected_rows)) {
    stop("Uttar Pradesh PAI scored-GP counts differ from the pinned release", call. = FALSE)
}
